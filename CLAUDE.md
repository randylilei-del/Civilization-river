...
tools/check.js        # 改完一条命令:build 一致性 → audit → 语法 → smoke → story-smoke;--quick 跳过后两项
...
tools/deadclick.js    # 「死点」扫描(v387,按需,不进 check.js:全跑约 14 分钟):八个视图里所有看起来能点的东西真点一遍,列出点了画面没反应的。判据是「7 岁孩子乱点十秒能自己悟出玩法」。改交互后跑;`--view 文明卡` 单跑;反例注入同 smoke(`SMOKE_INDEX`,副本放仓库目录);**一次只跑一个**(三个无头 Chrome 并行被系统按内存杀过)
...
han.html / song.html  #   中文单语、无主站入口(实验期有意不导流)。**只有 story-smoke 管它们能不能打开、点开始有没有反应**(v366.4 起);内容不在 audit/lint 范围内,
ming.html / qing.html #   也不在 sw.js 缓存清单、自身不注册 sw(离线打不开——v133 记的已知缺口)。
                      #   实验判据见 docs/TANG-EXPERIMENT.md;改内容不要指望工具兜底
