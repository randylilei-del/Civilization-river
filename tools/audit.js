...
const PEAK_NA = {
  split: ['古希腊', '玛雅', '战国', '春秋', '文艺复兴意大利', '粟特', '东晋·十六国', '五代十国',
          ...
          '印度-希腊诸王', '北元·蒙古诸部', '阿瓦·勃固', '托尔特克与谷地诸邦', '伐卡塔卡与德干诸国', '巽伽与恒河诸国', '帕查卡马克与中部海岸', '努比亚诸王国', '西藏诸政权',   /* v333:八百年无共主,教派与地方势力并立,没有单一版图数字 */
          /* v238.1:东察合台的理由与上面几条不同——1514—1560 的叶尔羌确实是个统一汗国、有版图可谈,
             留空的真正依据是 Taagepera 主序列没有这条,不是「内部碎成绿洲」(那描述的是结构不是可测性) */
          '东察合台·叶尔羌'],   /* v236:同期常有好几位国王各据一方,三十多个名字只见于钱币,没有单一版图数字 */
  agg:   ['印度·南亚诸国', '现代中东', '罗马治下近东', '澳新与太平洋岛国', '现代欧洲诸国',
          '海湾诸邦'],   /* v219:南岸是一排各管各的小港,不是一个政权,没有单一版图数字可取 */
  colo:  ['欧洲殖民非洲', '欧洲殖民东南亚', '西葡殖民美洲', '俄国治下中亚', '欧洲殖民大洋洲'],
  arch:  ['特奥蒂瓦坎', '吠陀时代', '二里头·夏', '奥尔梅克', '查文文化',
  ...
  /* v357.2:核查员问「整岛面积不是版图」这句为什么在斯里兰卡成立、在克里特不成立。
   答:斯里兰卡整岛从来不是一个政权——鼎盛期南北多国并存,拿整岛面积当版图是把不存在的
   统一硬安上去;而米诺斯的宫殿体系覆盖克里特全岛、岛上没有第二个体系与它并存,所以
   0.9 万 km²(≈克里特岛)是这条带的合理示意值。两条判词不矛盾,判据是「岛上有没有别人」。 */
...
const FIGURES_NA = [
  /* v357:米诺斯的理由是线形文字 A 至今未破译,这个文明没有任何人名传世(与二里头不写禹同理)。
     v357.2 更正:**迈锡尼不是这个理由**——线形文字 B 1952 年已破译,泥板里有人名(皮洛斯的
     Enkhelyawon,学界一般认为是位王),2015 年发掘的格里芬勇士墓还是个考古实证个体。
     按第 79 条的判据它落在「莫切的西潘王」那一格(可以立卡,待 Ray 拍板),不在「没有名字」那一格。
     暂留名单,但理由要写对——写错的理由会被后人当成定论继承,再没人敢碰。 */
  '迈锡尼', '米诺斯',
  '印度河文明', '三星堆·古蜀', '二里头·夏', '大津巴布韦',
  /* v244:中部海岸这一段没有名字传世的君主——利马文化与伊奇马都只留下遗址与陶器,
     神庙本身比任何一位统治者都出名。硬凑一个名字等于编。 */
  '帕查卡马克与中部海岸',
  ...
  '南马都尔'];
/* 规则 89(v382):城市照片的留空名单同样要被机器查。与规则 88 同一个洞、同一天发现:
 * 无照片的城 6 座,而 data/city_photo.js 文件头记的留空只有巴特那一座。多出的五座里,
 * 克诺索斯/以弗所/罗德岛是 v358 补七大奇迹时新入站,张掖/武威是写河西走廊时新入站——
 * 新城入站、选图没跟上,而没有任何东西会报。 */
const CITY_PHOTO_NA = [
  /* v202 判词:没有拿得准的图。 */
  '巴特那'];
{ const na = new Set(CITY_PHOTO_NA);
  const cityNames = new Set(GEO_CITY.map(g => g[0]));
  for (const n of na) if (!cityNames.has(n)) P.push(`[城市照片留空名单孤儿] ${n} 不是现存城市名`);
  for (const g of GEO_CITY) {
    const has = !!(CITY_PHOTO[g[0]] && CITY_PHOTO[g[0]].length);
    if (!has && !na.has(g[0])) P.push(`[城市缺照片且未列入留空名单] ${g[0]} —— 要么补照片,要么去 audit.js 的 CITY_PHOTO_NA 里说明为什么不该有`);
    if (has && na.has(g[0])) P.push(`[城市照片留空名单已过期] ${g[0]} 现在有照片,应从 CITY_PHOTO_NA 移除`);
  } }

/* 规则 88(v382):文明照片的留空名单要被机器查,不能只写在 data/civ_photo.js 的文件头注释里。
 * 由来:2026-09-06 盘点发现无照片的文明有 17 条,而注释里记的留空名单是 12 条(v209 的 11 条 + 喀布尔沙希)。
 * 多出的 5 条全是**拆带拆出来的新带**——米诺斯(v357 从米诺斯·迈锡尼拆出,狮子门留给了迈锡尼)、
 * 中王国埃及(v354)、高丽(v353)、幕府时代与近现代日本(日本拆四带)。老带的照片留在了另一半,
 * 新带静默加入留空,没有任何东西会报。这是第 40/41 条(拆带 checklist 要覆盖 tools/ 下的非 audit 输入)
 * 在**又一个文件**上的复发,而那两条教训的原话正是「立规则时先问:同型的错还能藏在哪个文件里?」
 * 判法照 FIGURES_NA / PEAK_NA:名单里有而现在有照片了 → 过期;没照片又不在名单 → 红。 */
const PHOTO_NA = [
  /* v209 判词:维基条目无首图、Commons 也搜不到可用实物照——要么真没有拿得出手的实物,
     要么条目首图是地图。这 11 条 + 喀布尔沙希(v342 判词:同上)是有据的留空。 */
  '匈奴', '柔然', '匈人', '嚈哒·白匈奴', '准噶尔', '俄国治下中亚',
  '西晋', '渤海国', '卡涅姆·博尔努', '加纳帝国', '拉皮塔·南岛航海者',
  '喀布尔沙希'];
{ const na = new Set(PHOTO_NA);
  for (const n of na) if (!names.has(n)) P.push(`[照片留空名单孤儿] ${n} 不是现存色带名`);
  CIVS.forEach(c => {
    const has = !!(CIV_PHOTO[c.n] && CIV_PHOTO[c.n].length);
    if (!has && !na.has(c.n)) P.push(`[缺照片且未列入留空名单] ${c.n} —— 要么补照片,要么去 audit.js 的 PHOTO_NA 里说明为什么不该有`);
    if (has && na.has(c.n)) P.push(`[照片留空名单已过期] ${c.n} 现在有照片,应从 PHOTO_NA 移除`);
  }); }
...
const LANDMARKS = [
  ['阿尔及尔', 3.06, 36.75, ['欧洲殖民非洲']],   // v249.2:「中心」点名的招牌殖民地,规则 72 抓到它在多边形外
  ...
  ['戈尔迪翁', 31.99, 39.65, ['弗里吉亚']],
  /* v325:锡诺普不在 GEO_CITY 里,而它是本都的都城与舰队母港 */
  ['锡诺普', 35.15, 42.03, ['本都王国']],
  /* v344:萨第斯不在 GEO_CITY 里,而它是吕底亚的都城 */
  ['萨第斯', 28.04, 38.48, ['吕底亚']],
  ['莫高窟', 94.81, 40.04, ['归义军']],   // v346:曹氏画院的现场
  /* v326:三坡布雷古不在 GEO_CITY 里,而它是真腊的都城与全带头号成就 */
  ['三坡布雷古', 105.05, 12.87, ['真腊']],
  /* v328:多摩梨帝不在 GEO_CITY 里,而它是早期孟加拉的头号成就 */
  ['多摩梨帝', 87.92, 22.30, ['早期孟加拉']],
  /* v329:卡米不在 GEO_CITY 里,而它是托尔瓦的都城与全带头号成就 */
  ['卡米', 28.37, -20.15, ['穆塔帕与托尔瓦']],
  /* v330:洪德与加兹尼城都不在 GEO_CITY 里,而它们是两条新带的都城 */
  ['乌德班达(洪德)', 72.43, 34.02, ['喀布尔沙希']],
  /* v333:札达的古格遗址不在 GEO_CITY 里,而它是西藏诸政权前半段的头号实物 */
  ['古格(札达)', 79.8, 31.48, ['西藏诸政权']],
  ['加兹尼城', 68.42, 33.55, ['加兹尼王朝']],
  /* v243:桑奇不在 GEO_CITY 里,而塔门正是这条带的核心成就 */
  ['桑奇', 77.74, 23.48, ['巽伽与恒河诸国']],
  /* v242:阿旃陀不在 GEO_CITY 里,靠地标探针守——第二期洞窟正是这条带的核心成就 */
  ['阿旃陀', 75.70, 20.55, ['伐卡塔卡与德干诸国']],
  /* v236:印度-希腊诸王的两个重镇,都不在 GEO_CITY 里,靠地标探针守 */
  ['坎大哈', 65.71, 31.61, ['印度-希腊诸王']],
  ['舍羯罗·锡亚尔科特', 74.53, 32.49, ['印度-希腊诸王']],
  ...
  ['朝鲜半岛南端', 126.8, 34.7, ['三国与统一新罗', '高丽', '朝鲜王朝', '韩国']],
  ['九州南端', 130.6, 31.3, ['古代日本','幕府时代','江户日本','近现代日本']],
  ...
  ['西奈半岛', 33.9, 29.5, ['新王国埃及', '罗马帝国', '拜占庭', '阿拉伯哈里发', '法蒂玛王朝', '马穆鲁克', '奥斯曼帝国', '现代中东']],
...
const NOT_IN = [
  ['张掖(甘州)', 100.45, 38.93, ['归义军']],   // v346:甘州归回鹘,不归归义军
  ['北京', 116.4, 39.9, ['鲜卑']],   // v345:檀石槐联盟在塞外,不入华北
  ['伊斯坦布尔', 28.98, 41.01, ['吕底亚']],   // v344:吕底亚不控博斯普鲁斯,拜占庭城不该点出吕底亚
  ['非斯', -5.0, 34.03, ['欧洲殖民非洲']],        // v249.2:摩洛哥归摩洛哥诸王朝,抬北缘不能顺手吞掉它
  ['的黎波里', 13.19, 32.9, ['欧洲殖民非洲']],   // v249.2:利比亚以东仍不画
  ['突尼斯', 10.18, 36.8, ['欧洲殖民非洲']],     // v249.2:突尼斯的保护国那段住在 突尼斯|现代中东,不重讲
  ['亚的斯亚贝巴', 38.74, 9.03, ['扎格维王朝']],   // v248:高原南部是后来所罗门王朝南扩的范围
  ['麦罗埃', 33.72, 16.94, ['扎格维王朝']],   // v248:努比亚有自己的带
  ['华沙', 21.01, 52.23, ['勃兰登堡·普鲁士']],   // v247:1795 瓜分所得有意不画,那块与波兰-立陶宛重叠太多
  ['维也纳', 16.37, 48.21, ['勃兰登堡·普鲁士']],   // v247:哈布斯堡有自己的带
  ['萨迪斯', 28.04, 38.49, ['弗里吉亚']],
  ['安卡拉', 32.85, 39.93, ['本都王国']],   // v325:米特里达梯六世战时一度深入内陆,转瞬即失——只画长期本土,带名与几何一致   // v246:那是吕底亚的都城,本带止于弗里吉亚亡国——带名与几何必须一致(v238.1 别失八里的教训)
  ['开罗', 31.24, 30.04, ['努比亚诸王国']],   // v245:埃及本土不画,baqt 的两端正是这条边界
  ['阿克苏姆', 38.72, 14.13, ['努比亚诸王国']],   // v245:埃塞俄比亚高原各有其带
  ['库斯科', -71.97, -13.52, ['帕查卡马克与中部海岸']],   // v244:高原归印加/瓦里,本带只画中部海岸河谷
  ['加尔各答', 88.36, 22.57, ['巽伽与恒河诸国']],   // v243:恒河下游这几百年没有站内可对应的政权,巽伽的控制证据也不硬,有意留在外
  ['达卡', 90.41, 23.81, ['巽伽与恒河诸国']],   // v243:同上
  ['金奈', 80.27, 13.08, ['伐卡塔卡与德干诸国']],   // v242:泰米尔归朱罗一线,不是德干
  ['瓦拉纳西', 83.01, 25.32, ['伐卡塔卡与德干诸国']],   // v242:恒河流域归笈多
  ['奇琴伊察', -88.57, 20.68, ['托尔特克与谷地诸邦']],   // v241:归玛雅带——图拉与奇琴伊察建筑几乎相同但谁影响谁无定论,不用版图替学界下结论
  ['曼谷', 100.5, 13.76, ['阿瓦·勃固','真腊']],   // v240:暹罗有自己的带;v326:昭披耶河流域是堕罗钵底诸邦,不画进真腊
  ['佛罗伦萨', 11.25, 43.77, ['教皇国']],   // v327:托斯卡纳不是教皇国
  ['那不勒斯', 14.25, 40.85, ['教皇国']],   // v327:南意归那不勒斯王国
  ['博洛尼亚', 11.34, 44.49, ['教皇国']],   // v327:1506 年后才入教皇国,画进来会把 754—1506 一并声索
  ['巴特那', 85.14, 25.61, ['早期孟加拉']],   // v328:摩揭陀归巽伽/笈多一线,设赏迦短暂西进不画
  ['索法拉', 34.74, -20.16, ['穆塔帕与托尔瓦']],   // v329:黄金出海口归斯瓦希里城邦,内陆带不画到海
  ['木尔坦', 71.47, 30.2, ['喀布尔沙希']],   // v330:木尔坦是独立埃米尔国,不归沙希
  ['赫拉特', 62.2, 34.35, ['加兹尼王朝']],   // v330:呼罗珊只握四十来年,不画(教皇国不吞博洛尼亚同理)
  ['加德满都', 85.32, 27.71, ['西藏诸政权']],   // v333:谷地是尼泊尔诸王朝的地盘,不画
  ['吴哥', 103.87, 13.41, ['阿瓦·勃固']],   // v240:高棉有自己的带
  ['摩苏尔', 43.14, 36.34, ['加喜特巴比伦']],   // v239:尼尼微一带是亚述核心,不是巴比伦尼亚
  ['阿拉木图', 76.89, 43.24, ['东察合台·叶尔羌']],   // v238:七河归哈萨克汗国
  ['撒马尔罕', 66.96, 39.65, ['东察合台·叶尔羌']],   // v238:河中归布哈拉三汗国
  ['北京', 116.4, 39.9, ['北元·蒙古诸部']],   // v237:长城以南不画,土木堡之后也没能占住
  ['敦煌', 94.66, 40.14, ['北元·蒙古诸部']],   // v237:河西是明的羁縻卫所与后来的吐鲁番,不是北元
  ['德里', 77.21, 28.61, ['印度-希腊诸王']],   // v236:米南德的军队一度东进恒河,但未长期占有
  ['撒马尔罕', 66.96, 39.65, ['印度-希腊诸王']],   // v236:河中是希腊-巴克特里亚,本带只画翻过兴都库什之后的部分
  /* v235 A类第四轮的反向探针 */
 ...
 */
const GEO_CITIES = [
  /* v249.2:基线一次性并入当时全部「新增命中」(71 城,多为 v211 现代欧洲诸国 / v230 伊比利亚 / 近两周新带),
     从此规则 36 对新增命中报 warn——新带盖到基线外的城,要么有意加进来,要么是画多了。 */
  ["克诺索斯",25.16,35.30,["米诺斯","迈锡尼","古希腊","罗马共和国","罗马帝国","拜占庭","奥斯曼帝国"]],   /* v357.2:拆带后米诺斯在基线里一座城都盖不到,等于从地图那侧没有入口 */
  ["伦敦",-0.13,51.51,["中世纪西欧","大英帝国·英国","罗马帝国"]],
  ["巴黎",2.35,48.86,["中世纪西欧","法兰西","罗马共和国","罗马帝国"]],
  ["罗马",12.5,41.9,["中世纪西欧","拜占庭","文艺复兴意大利","罗马共和国","罗马帝国","现代欧洲诸国","教皇国"]],
  ["雅典",23.73,37.98,["古希腊","奥斯曼帝国","拜占庭","迈锡尼","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["马德里",-3.7,40.42,["中世纪西欧","后倭马亚·安达卢斯","罗马共和国","罗马帝国","西班牙·葡萄牙帝国","现代欧洲诸国","阿拉伯哈里发"]],
  ["里斯本",-9.14,38.72,["后倭马亚·安达卢斯","罗马共和国","罗马帝国","西班牙·葡萄牙帝国","中世纪西欧","现代欧洲诸国","阿拉伯哈里发"]],
  ["科尔多瓦",-4.78,37.89,["后倭马亚·安达卢斯","罗马共和国","罗马帝国","西班牙·葡萄牙帝国","中世纪西欧","现代欧洲诸国","阿拉伯哈里发"]],
  ["柏林",13.4,52.52,["中世纪西欧","勃兰登堡·普鲁士","德意志"]],
  ["维也纳",16.37,48.21,["奥地利·哈布斯堡","罗马帝国","中世纪西欧","现代欧洲诸国"]],
  ["布拉格",14.42,50.09,["中世纪西欧","德意志","现代欧洲诸国","奥地利·哈布斯堡"]],
  ["华沙",21.01,52.23,["德意志","波兰-立陶宛","现代欧洲诸国"]],
  ["维尔纽斯",25.28,54.69,["俄罗斯·苏联","波兰-立陶宛","现代欧洲诸国"]],
  ["斯德哥尔摩",18.07,59.33,["瑞典帝国","维京·北欧","现代欧洲诸国","北欧诸王国"]],
  ["奥斯陆",10.75,59.91,["维京·北欧","现代欧洲诸国","北欧诸王国"]],
  ["哥本哈根",12.57,55.68,["维京·北欧","现代欧洲诸国","北欧诸王国"]],
  ["阿姆斯特丹",4.9,52.37,["中世纪西欧","荷兰共和国","现代欧洲诸国"]],
  ["布鲁塞尔",4.35,50.85,["中世纪西欧","法兰西","罗马帝国","现代欧洲诸国"]],
  ["米兰",9.19,45.46,["中世纪西欧","法兰西","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["威尼斯",12.34,45.44,["中世纪西欧","文艺复兴意大利","法兰西","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["佛罗伦萨",11.26,43.77,["中世纪西欧","文艺复兴意大利","法兰西","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["那不勒斯",14.27,40.85,["拜占庭","文艺复兴意大利","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ...
  ["都柏林",-6.26,53.35,["中世纪西欧","大英帝国·英国","维京·北欧","现代欧洲诸国"]],
  ["爱丁堡",-3.19,55.95,["中世纪西欧","大英帝国·英国"]],
  ["日内瓦",6.14,46.2,["中世纪西欧","法兰西","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["慕尼黑",11.58,48.14,["中世纪西欧","奥地利·哈布斯堡","德意志","法兰西","罗马帝国"]],
  ["布达佩斯",19.04,47.5,["匈人","奥地利·哈布斯堡","罗马帝国","现代欧洲诸国"]],
  ["贝尔格莱德",20.46,44.79,["奥斯曼帝国","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["索菲亚",23.32,42.7,["奥斯曼帝国","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["克拉科夫",19.94,50.06,["德意志","波兰-立陶宛","现代欧洲诸国"]],
  ["里加",24.11,56.95,["俄罗斯·苏联","瑞典帝国","现代欧洲诸国"]],
  ["巴塞罗那",2.17,41.39,["罗马共和国","罗马帝国","西班牙·葡萄牙帝国","中世纪西欧","现代欧洲诸国","阿拉伯哈里发"]],
  ["巴格达",44.36,33.31,["亚述","伊尔汗国","加喜特巴比伦","古巴比伦","塞尔柱突厥","奥斯曼帝国","希腊化王朝","帕提亚·萨珊","新巴比伦","波斯(阿契美尼德)","现代中东","罗马帝国","苏美尔·阿卡德","萨法维·波斯","阿拉伯哈里发"]],
  ["大马士革",36.29,33.51,["亚述","新王国埃及","奥斯曼帝国","希腊化王朝","拜占庭","新巴比伦","法蒂玛王朝","波斯(阿契美尼德)","现代中东","罗马帝国","罗马治下近东","阿尤布王朝","阿拉伯哈里发","马穆鲁克"]],
  ["耶路撒冷",35.21,31.77,["亚述","新王国埃及","奥斯曼帝国","希腊化王朝","拜占庭","新巴比伦","法蒂玛王朝","波斯(阿契美尼德)","现代中东","罗马帝国","罗马治下近东","阿尤布王朝","阿拉伯哈里发","马穆鲁克"]],
  ["开罗",31.24,30.04,["亚述","新王国埃及","奥斯曼帝国","拜占庭","法蒂玛王朝","波斯(阿契美尼德)","现代中东","罗马共和国","罗马帝国","罗马治下近东","阿尤布王朝","阿拉伯哈里发","马穆鲁克","古王国埃及","中王国埃及"]],  // v354:古埃及拆三带后,吉萨就在开罗边上,三条带都盖到
  ...
  ["巴比伦",44.42,32.54,["亚述","伊尔汗国","加喜特巴比伦","古巴比伦","塞尔柱突厥","奥斯曼帝国","希腊化王朝","帕提亚·萨珊","新巴比伦","波斯(阿契美尼德)","现代中东","苏美尔·阿卡德","萨法维·波斯","阿拉伯哈里发"]],
  ["乌尔",46.1,30.96,["亚述","伊尔汗国","加喜特巴比伦","古巴比伦","奥斯曼帝国","希腊化王朝","帕提亚·萨珊","新巴比伦","波斯(阿契美尼德)","现代中东","苏美尔·阿卡德","萨法维·波斯","阿拉伯哈里发"]],
  ...
  ["喀布尔",69.17,34.53,["印度-希腊诸王","嚈哒·白匈奴","孔雀王朝","帖木儿帝国","波斯(阿契美尼德)","莫卧儿帝国","萨曼王朝","贵霜帝国","阿拉伯哈里发","阿富汗","喀布尔沙希","加兹尼王朝","蒙古帝国"]],  // v343:喀布尔在蒙古/察合台兀鲁思治下,版图顶点[70,35]→[67,33.8]补上,关 1258—1370 断层
  ["巴尔米拉",38.27,34.55,["亚述","奥斯曼帝国","希腊化王朝","拜占庭","新巴比伦","波斯(阿契美尼德)","现代中东","罗马帝国","罗马治下近东","阿拉伯哈里发"]],
  ["特洛伊",26.24,39.96,["古希腊","塞尔柱突厥","奥斯曼帝国","拜占庭","现代中东","罗马共和国","罗马帝国","罗马治下近东"]],
  ["突尼斯",10.32,36.85,["奥斯曼帝国","拜占庭","现代中东","罗马共和国","罗马帝国","迦太基"]],
  ["非斯",-5,34.03,["现代中东","罗马共和国","罗马帝国","迦太基","阿拉伯哈里发","摩洛哥诸王朝"]],
  ["马拉喀什",-8.01,31.63,["现代中东","阿拉伯哈里发","摩洛哥诸王朝"]],
  ["的黎波里",13.19,32.89,["奥斯曼帝国","拜占庭","法蒂玛王朝","现代中东","罗马共和国","罗马帝国","阿拉伯哈里发"]],
  ["阿尔及尔",3.06,36.75,["拜占庭","罗马共和国","罗马帝国","迦太基","阿拉伯哈里发","欧洲殖民非洲"]],
  ["昔兰尼",21.86,32.82,["奥斯曼帝国","拜占庭","现代中东","罗马共和国","罗马帝国"]],
  ["萨那",44.21,15.35,["现代中东","阿尤布王朝","阿拉伯哈里发"]],
  ["特拉布宗",39.72,41,["塞尔柱突厥","奥斯曼帝国","拜占庭","波斯(阿契美尼德)","现代中东","罗马帝国","本都王国"]],
  ...
  ["安卡拉",32.85,39.93,["塞尔柱突厥","奥斯曼帝国","弗里吉亚","拜占庭","波斯(阿契美尼德)","现代中东","罗马共和国","罗马帝国","罗马治下近东","赫梯","吕底亚"]],  // v344:克罗伊斯治下东到哈吕斯,安卡拉在界内,关 前676—前550 断层
  ["德里",77.21,28.61,["中世纪诸王朝","印度·南亚诸国","印度河文明","吠陀时代","大英帝国·英国","孔雀王朝","巽伽与恒河诸国","德里苏丹国","笈多王朝","英属印度","莫卧儿帝国","贵霜帝国","马拉塔"]],
  ["孟买",72.88,19.08,["中世纪诸王朝","伐卡塔卡与德干诸国","印度·南亚诸国","大英帝国·英国","孔雀王朝","百乘王朝","英属印度","马拉塔","德里苏丹国"]],
  ["加尔各答",88.36,22.57,["印度·南亚诸国","大英帝国·英国","孔雀王朝","波罗王朝","英属印度","莫卧儿帝国","笈多王朝","德里苏丹国","早期孟加拉"]],
  ["金奈",80.27,13.08,["印度·南亚诸国","大英帝国·英国","朱罗王朝","维查耶纳伽尔","英属印度"]],
  ["摩亨佐达罗",68.14,27.33,["印度-希腊诸王","印度·南亚诸国","印度河文明","孔雀王朝","波斯(阿契美尼德)","英属印度","莫卧儿帝国","贵霜帝国","阿拉伯哈里发","阿富汗","德里苏丹国"]],
  ["哈拉帕",72.87,30.63,["印度-希腊诸王","印度·南亚诸国","印度河文明","吠陀时代","嚈哒·白匈奴","孔雀王朝","德里苏丹国","英属印度","莫卧儿帝国","贵霜帝国","阿富汗","加兹尼王朝"]],
  ["巴特那",85.14,25.61,["印度·南亚诸国","大英帝国·英国","孔雀王朝","巽伽与恒河诸国","德里苏丹国","波罗王朝","笈多王朝","英属印度","莫卧儿帝国","马拉塔","中世纪诸王朝"]],
  ["瓦拉纳西",83.01,25.32,["中世纪诸王朝","印度·南亚诸国","大英帝国·英国","孔雀王朝","巽伽与恒河诸国","德里苏丹国","笈多王朝","英属印度","莫卧儿帝国","马拉塔"]],
  ["坎大哈",65.71,31.61,["帕提亚·萨珊","帖木儿帝国","波斯(阿契美尼德)","贵霜帝国","阿拉伯哈里发","阿富汗","孔雀王朝","印度-希腊诸王","莫卧儿帝国"]],
  ["塔克西拉",72.83,33.74,["印度-希腊诸王","印度·南亚诸国","嚈哒·白匈奴","孔雀王朝","英属印度","莫卧儿帝国","贵霜帝国","阿富汗","喀布尔沙希","加兹尼王朝"]],
  ["汉皮",76.46,15.34,["中世纪诸王朝","伐卡塔卡与德干诸国","印度·南亚诸国","大英帝国·英国","孔雀王朝","百乘王朝","维查耶纳伽尔","英属印度"]],
  ["马杜赖",78.12,9.93,["印度·南亚诸国","朱罗王朝","维查耶纳伽尔","英属印度"]],
  ["拉合尔",74.34,31.55,["印度-希腊诸王","印度·南亚诸国","印度河文明","吠陀时代","嚈哒·白匈奴","孔雀王朝","英属印度","莫卧儿帝国","贵霜帝国","阿富汗","德里苏丹国","喀布尔沙希","加兹尼王朝"]],
  ["达卡",90.41,23.81,["印度·南亚诸国","大英帝国·英国","孔雀王朝","波罗王朝","英属印度","莫卧儿帝国","早期孟加拉"]],
  ["科伦坡",79.86,6.93,["朱罗王朝","斯里兰卡诸王国","英属印度","印度·南亚诸国"]],
  ...
  ["杭州",120.15,30.27,["三国","东晋·十六国","东汉","五代十国","元","共和国","北宋","南北朝","南宋","唐","明","民国","清","秦","蒙古帝国","西晋","西汉","隋","春秋","战国"]],
  ["开封",114.31,34.8,["三国","东汉","二里头·夏","五代十国","元","共和国","北宋","南北朝","唐","商","战国","明","春秋","民国","清","秦","蒙古帝国","西周","西晋","西汉","金","隋","东晋·十六国"]],
  ["成都",104.07,30.57,["三国","东晋·十六国","东汉","五代十国","元","共和国","北宋","南北朝","南宋","唐","战国","明","民国","清","秦","蒙古帝国","西晋","西汉","隋","三星堆·古蜀"]],
  ...
  ["拉萨",91.14,29.65,["元","共和国","吐蕃","民国","清","蒙古帝国","西藏诸政权"]],
  ["乌鲁木齐",87.62,43.83,["东察合台·叶尔羌","元","共和国","准噶尔","唐","民国","清","蒙古帝国"]],
  ["敦煌",94.66,40.14,["三国","东察合台·叶尔羌","东汉","元","共和国","南北朝","吐蕃","唐","民国","清","蒙古帝国","西夏","西晋","西汉","隋","东晋·十六国","归义军"]],  // v346:关 907—1038 断层
  ["喀什",75.99,39.47,["东察合台·叶尔羌","共和国","唐","喀喇汗王朝","民国","清","蒙古帝国","西辽·哈剌契丹"]],
  ["呼和浩特",111.75,40.84,["东汉","元","共和国","北元·蒙古诸部","南北朝","唐","明","民国","清","秦","蒙古帝国","西汉","辽","金","隋","东晋·十六国"]],
  ["哈尔滨",126.53,45.8,["俄罗斯·苏联","元","清","渤海国","蒙古帝国","辽","金"]],
  ["首尔",126.98,37.57,["三国与统一新罗","高丽","朝鲜王朝","韩国"]],
  ["平壤",125.75,39.03,["三国与统一新罗","高丽","朝鲜王朝","蒙古帝国","朝鲜"]],
  ["庆州",129.22,35.86,["三国与统一新罗","高丽","朝鲜王朝","韩国"]],
  ["东京",139.69,35.69,["古代日本","幕府时代","江户日本","近现代日本"]],
  ["京都",135.77,35.01,["古代日本","幕府时代","江户日本","近现代日本"]],
  ["大阪",135.5,34.69,["古代日本","幕府时代","江户日本","近现代日本"]],
  ["福冈",130.4,33.59,["古代日本","幕府时代","江户日本","近现代日本"]],
  ["札幌",141.35,43.06,["古代日本","幕府时代","江户日本","近现代日本"]],
  ["台北",121.56,25.03,["共和国","民国","清"]],
  ["香港",114.17,22.32,["三国","东晋·十六国","东汉","五代十国","元","共和国","北宋","南北朝","南宋","唐","大英帝国·英国","明","民国","清","秦","西晋","西汉","隋"]],
  ["乌兰巴托",106.92,47.92,["元","匈奴","北元·蒙古诸部","回鹘","柔然","清","突厥汗国","蒙古帝国","辽","蒙古国","鲜卑"]],  // v345:匈奴→柔然之间的正主,关 200—330 断层
  ["哈拉和林",102.83,47.2,["元","匈奴","北元·蒙古诸部","回鹘","柔然","清","突厥汗国","蒙古帝国","辽","蒙古国","鲜卑"]],  // v345:匈奴→柔然之间的正主,关 200—330 断层
  ...
  ["吴哥",103.87,13.41,["欧洲殖民东南亚","现代东南亚","高棉·吴哥","真腊"]],
  ["曼谷",100.5,13.76,["暹罗","欧洲殖民东南亚","现代东南亚","高棉·吴哥"]],
  ["素可泰",99.82,17.01,["暹罗","欧洲殖民东南亚","现代东南亚"]],
  ["蒲甘",94.86,21.17,["欧洲殖民东南亚","现代东南亚","蒲甘","阿瓦·勃固"]],
  ["仰光",96.16,16.87,["欧洲殖民东南亚","现代东南亚","蒲甘","阿瓦·勃固"]],
  ...
  ["金边",104.92,11.56,["欧洲殖民东南亚","现代东南亚","高棉·吴哥","真腊"]],
  ["胡志明市",106.66,10.82,["扶南","真腊","高棉·吴哥","欧洲殖民东南亚","现代东南亚"]],   // v326.1:核查员指出此城此前不在基线,新增命中规则守不到——补上
  ["阿克苏姆",38.72,14.13,["埃塞俄比亚帝国","扎格维王朝","欧洲殖民非洲","现代非洲","阿克苏姆"]],
  ...
  ["大津巴布韦",30.93,-20.27,["大津巴布韦","欧洲殖民非洲","现代非洲","穆塔帕与托尔瓦"]],
  ["基尔瓦",39.55,-8.96,["斯瓦希里城邦","欧洲殖民非洲","现代非洲"]],
  ["蒙巴萨",39.67,-4.04,["斯瓦希里城邦","欧洲殖民非洲","现代非洲"]],
  ["开普敦",18.42,-33.92,["大英帝国·英国","欧洲殖民非洲","现代非洲"]],
  ["麦罗埃",33.72,16.94,["努比亚诸王国","库施·努比亚","欧洲殖民非洲","现代非洲"]],
  ["喀土穆",32.53,15.5,["库施·努比亚","欧洲殖民非洲","现代非洲","努比亚诸王国"]],
  ["金沙萨",15.31,-4.32,["欧洲殖民非洲","现代非洲"]],
  ["罗安达",13.23,-8.84,["欧洲殖民非洲","现代非洲"]],
  ["特奥蒂瓦坎",-98.84,19.69,["托尔特克与谷地诸邦","拉丁美洲诸国","特奥蒂瓦坎","西班牙·葡萄牙帝国","西葡殖民美洲","阿兹特克"]],
  ["墨西哥城",-99.13,19.43,["托尔特克与谷地诸邦","拉丁美洲诸国","特奥蒂瓦坎","西班牙·葡萄牙帝国","西葡殖民美洲","阿兹特克"]],
  ...
  ["利马",-77.04,-12.05,["帕查卡马克与中部海岸","拉丁美洲诸国","查文文化","西班牙·葡萄牙帝国","西葡殖民美洲"]],
  ["蒂瓦纳科",-68.67,-16.55,["拉丁美洲诸国","瓦里·蒂瓦纳科","西葡殖民美洲"]],
  ["卡拉尔",-77.52,-10.89,["奇穆","拉丁美洲诸国","查文文化","西班牙·葡萄牙帝国","西葡殖民美洲","帕查卡马克与中部海岸"]],
  ...
  ["萨尔瓦多",-38.51,-12.97,["西班牙·葡萄牙帝国","西葡殖民美洲","拉丁美洲诸国"]],
  ["卡霍基亚",-90.06,38.66,["密西西比文化·卡霍基亚","美国"]],
  ["查科峡谷",-107.96,36.06,["美国"]],
  ["纽约",-74.01,40.71,["美国","荷兰共和国"]],
  ["华盛顿",-77.04,38.91,["美国"]],
  ["波士顿",-71.06,42.36,["美国"]],
  ["洛杉矶",-118.24,34.05,["美国"]],
  ["蒙特利尔",-73.57,45.5,["大英帝国·英国"]],
  ["哈瓦那",-82.38,23.13,["拉丁美洲诸国","西葡殖民美洲"]],
  ["悉尼",151.21,-33.87,["大英帝国·英国","澳大利亚原住民","欧洲殖民大洋洲","澳新与太平洋岛国"]],
  ["奥克兰",174.76,-36.85,["大英帝国·英国","毛利","欧洲殖民大洋洲","澳新与太平洋岛国"]],
  ["火奴鲁鲁",-157.86,21.31,["夏威夷王国","欧洲殖民大洋洲"]],
  ["复活节岛",-109.35,-27.12,["拉帕努伊·复活节岛"]],
  ["圣彼得堡",30.31,59.94,["俄罗斯·苏联"]],
  ["喀山",49.11,55.79,["俄罗斯·苏联"]],
  ["阿斯特拉罕",48.03,46.35,["俄罗斯·苏联","斯基泰","蒙古帝国"]],
  ["塔什干",69.24,41.3,["俄国治下中亚","喀喇汗王朝","布哈拉·希瓦·浩罕三汗国","现代中亚","花剌子模","蒙古帝国","粟特"]],
  ...
  ["海口",110.2,20.04,["东汉","元","共和国","北宋","南宋","唐","明","民国","清","西汉","隋","五代十国"]],
  ...
  ["济南",117.12,36.65,["三国","东汉","五代十国","元","共和国","南北朝","唐","商","战国","明","春秋","民国","清","秦","蒙古帝国","西晋","西汉","金","隋","东晋·十六国","北宋"]],
  ["太原",112.55,37.87,["三国","东汉","五代十国","元","共和国","南北朝","唐","战国","明","春秋","民国","清","秦","蒙古帝国","西晋","西汉","金","隋","北宋","东晋·十六国"]],   // v203:去掉「辽」(辽从未统治太原);v203.1 加「北宋」(河东路治所)
  ["兰州",103.83,36.06,["三国","东汉","五代十国","元","共和国","南北朝","唐","明","民国","清","秦","蒙古帝国","西夏","西晋","西汉","隋","东晋·十六国"]],
  ["西宁",101.78,36.62,["三国","东汉","元","共和国","吐蕃","唐","明","民国","清","秦","蒙古帝国","西夏","西晋","西汉","隋","东晋·十六国"]],
  ["银川",106.23,38.49,["三国","东汉","五代十国","元","共和国","南北朝","唐","明","民国","清","秦","蒙古帝国","西夏","西晋","西汉","金","隋","东晋·十六国"]],
  ["沈阳",123.43,41.8,["东汉","元","共和国","唐","明","民国","清","蒙古帝国","西汉","辽","金","东晋·十六国"]],
  ["大连",121.62,38.91,["三国","东汉","元","共和国","南北朝","唐","战国","明","民国","清","蒙古帝国","西晋","西汉","辽","金","隋","东晋·十六国"]],
  ["釜山",129.08,35.18,["三国与统一新罗","高丽","朝鲜王朝","韩国"]],
  ["长春",125.32,43.82,["元","清","渤海国","蒙古帝国","辽","金"]],
  ["迈阿密",-80.19,25.76,["美国","西葡殖民美洲"]],
  ["圣迭戈",-117.16,32.72,["美国"]],
  ["旧金山",-122.42,37.77,["美国"]],
  ["西雅图",-122.33,47.61,["美国"]],
  ...
  ["亚丁",45.03,12.79,["现代中东","阿尤布王朝","阿拉伯哈里发"]],
  ["摩加迪沙",45.34,2.05,["欧洲殖民非洲","现代非洲"]],
  ["哈拉雷",31.05,-17.83,["欧洲殖民非洲","现代非洲","穆塔帕与托尔瓦"]],
  ["卢萨卡",28.28,-15.42,["欧洲殖民非洲","现代非洲"]],
  ["塞维利亚",-5.98,37.39,["后倭马亚·安达卢斯","罗马共和国","罗马帝国","西班牙·葡萄牙帝国","现代欧洲诸国","阿拉伯哈里发"]],
  ["图卢兹",1.44,43.6,["中世纪西欧","法兰西","罗马共和国","罗马帝国"]],
  ["科隆",6.96,50.94,["中世纪西欧","德意志","法兰西","罗马帝国"]],
  ["汉堡",9.99,53.55,["中世纪西欧","德意志"]],
  ["赫尔辛基",24.94,60.17,["俄罗斯·苏联","瑞典帝国","现代欧洲诸国","北欧诸王国"]],
  ["塔林",24.75,59.44,["俄罗斯·苏联","瑞典帝国","现代欧洲诸国"]],
  ["雅西",27.59,47.16,["匈人","现代欧洲诸国"]],
  ["塞萨洛尼基",22.94,40.63,["古希腊","奥斯曼帝国","拜占庭","罗马共和国","罗马帝国","现代欧洲诸国"]],
  ["敖德萨",30.73,46.48,["俄罗斯·苏联","匈人"]],
  ["塞瓦斯托波尔",33.53,44.62,["俄罗斯·苏联","奥斯曼帝国","罗马帝国","蒙古帝国"]],
  ["卡萨布兰卡",-7.59,33.57,["现代中东","阿拉伯哈里发","摩洛哥诸王朝"]]
];
GEO_CITIES.forEach(([cn, x, y, base]) => {
  ...
  /* v249.2(warn):新增命中也说一声。v230 为纳入伊比利亚顺手把格拉纳达吞进中世纪西欧、v233.1 把和田吞进元,
     规则 36 都沉默——它只报丢失。加新带让命中变多是正常的,所以不 fail;但「谁新盖到了这座城」必须被看见,
     确认有意就把它写进基线,这条 warn 就消失。 */
  const gained = hit.filter(n => !base.includes(n));
  if (gained.length)
    W.push(`[版图新增命中] ${cn}: 「${gained.join('、')}」现在盖到了这座城,基线里没有——有意就加进 GEO_CITIES 基线,无意就是画多了`);
});

/* 72. 招牌不能在自己边界外(v249.2)。五轮核查里复发 4 次(坎大哈/别失八里/法拉斯/西里西亚):速览「中心」
 * 点名的地方,版图多边形却没盖住——卡片说这是我的地方,地图上却在别人家。「中心」的每个地名按
 * PLACE 表换算成今日城市名,再到 GEO_CITY / GEO_CITIES 基线 / LANDMARKS / NOT_IN 里找坐标;找得到坐标
 * 就做 PIP,不在自己任何一个多边形内就红。找不到坐标的地名(波斯波利斯这类没有城市点的遗址)跳过——
 * 这条规则只保证「站内已有坐标的地方」不打架,不假装覆盖全部。 */
{
  const coord = new Map();
  (GEO_CITY || []).forEach(([zh, , lon, lat]) => coord.set(zh, [lon, lat]));
  GEO_CITIES.forEach(([cn, x, y]) => { if (!coord.has(cn)) coord.set(cn, [x, y]); });
  [...(typeof LANDMARKS !== 'undefined' ? LANDMARKS : []), ...(typeof NOT_IN !== 'undefined' ? NOT_IN : [])]
    .forEach(([nm, x, y]) => { if (!coord.has(nm)) coord.set(nm, [x, y]); });
  const strip = s => s.replace(/[(（].*?[)）]/g, '').trim();
  let checked = 0;
  CIVS.forEach(c => {
    const g = GEO[c.n]; if (!g || !g.p) return;
    const v = c.f && c.f['中心']; if (!v) return;
    v.split(SPLIT).map(s => s.trim()).filter(Boolean).forEach(p => {
      const t = PLACE[p];
      const cands = [p, t && t[2], strip(p)].filter(Boolean);   // 先按 PLACE 换算的今日城市名找(辽的「南京(北京)」是北京不是南京),再退到去括号的原名
      const key = cands.find(k => coord.has(k)); if (!key) return;
      checked++;
      const [x, y] = coord.get(key);
      if (!g.p.some(poly => pip(x, y, poly)))
        P.push(`[中心在自己版图外] ${c.n} 的「中心」写了 ${p}(按 ${key} ${x},${y}),但 GEO['${c.n}'] 没有一个多边形盖住它——要么补画,要么别把它写进「中心」`);
    });
  });
  if (!checked) P.push(`[规则 72 失明] 一个「中心」地名都没换算出坐标,检查 PLACE/GEO_CITY 结构是否变了`);
}
 ...
 * 「今 蒙古国:哈拉和林」,而 build / audit / 语法 / smoke 一个都不会红:新键有值、旧键不报孤儿、
 * 渲染也不出 undefined。这类「新加一条静默盖掉既有一条」是可以反复犯的模板缺陷,所以加规则。
 * 判据:逐个 data 文件扫顶层对象的 'key': 字面量,同名出现两次即报。 */
const fs_ = require('fs'), path_ = require('path');
for (const f of fs_.readdirSync(path_.join(__dirname, '../data')).filter(x => x.endsWith('.js'))) {
  const src_ = fs_.readFileSync(path_.join(__dirname, '../data', f), 'utf8');
  const seen_ = new Map();
  for (const m of src_.matchAll(/^\s*'([^']+)'\s*:/gm)) {
    const k = m[1];
    if (seen_.has(k)) P.push(`[键重复] data/${f} 里 '${k}' 出现 ${seen_.get(k) + 1} 次 —— JS 后写胜出,前一条被静默覆盖`);
    seen_.set(k, (seen_.get(k) || 0) + 1);
  }
}

/* 规则 69(v236.1):叙述段 d 必须中英都有。
 * 由来:v236 新加的「印度-希腊诸王」只写了 e.d,没写中文 d —— 160 条里唯一一条,
 * 而 index.html 的渲染是 `LANG==='zh' ? (c.d || c.b) : c.e.d`,**缺中文 d 会静默回退到
 * 一句话的 b**:不报错、不留白、不产生 undefined,所以「N 卡零 undefined/NaN」这条自检
 * 结构上就抓不到它。结果是中文读者点开这张卡,比英文读者整整少一段。
 * v237 的「北元·蒙古诸部」当场复发了同一个漏洞 —— 一次是手滑,两次是模板缺陷,所以加规则。
 * 先清零再加:加规则时 161 条 d / e.d 均已补齐(v236.1 核对)。 */
CIVS.forEach(c => {
  if (!c.d) P.push(`[叙述段缺中文] ${c.n} 没有 d —— 渲染会静默回退到一句话的 b,中文读者少一整段`);
  if (!c.e || !c.e.d) P.push(`[叙述段缺英文] ${c.n} 没有 e.d`);
});
...
const AREA_OK = [
  ...
  '近现代日本',  /* v352:日本拆四带后,这条豁免归近现代那一条(PEAK 740 万含 1942 占领区,GEO 只画本土) */
];
{
  const sphArea = polys => {
 ...
 * 文化/文明/时代/治下/属/·/-),且色带在 2000 年前结束 → 红。 */
{
  ...
  const QUAL = /古|早期|帝国|王国|王朝|诸|共和国|殖民|苏丹|汗国|城邦|文化|文明|时代|治下|属|·|-|人|化|北元|文艺复兴|江户/;   // v328:「早期」同样告诉读者这是那个国家的某一段(早期孟加拉)
   ...
   (第二十一轮核查抓到三条指向已删带名的映射)。下次重生成会照着它把旧键写回来。 */
{
  ...
  /* v357.2:上一版只守住 dynasty_map 那一个文件,第二十二轮核查在同目录的兄弟文件
     civ_photo_curated_titles.json / titles2.json 里抓到同型残留(拆米诺斯·迈锡尼时漏改)。
     这两张是数组表,每项第 0 位是带名。教训:守单个文件名等于没守,要守一类文件。 */
  for (const fn of fs2.readdirSync(__dirname)) {
    if (!/^civ_photo_.*\.json$/.test(fn) || fn === 'civ_photo_dynasty_map.json') continue;
    let rows; try { rows = JSON.parse(fs2.readFileSync(pth.join(__dirname, fn), 'utf8')); } catch { continue; }
    if (!Array.isArray(rows)) continue;
    for (const r of rows)
      if (Array.isArray(r) && typeof r[0] === 'string' && !names.has(r[0]))
        P.push(`[照片选题表带名不是现存色带] ${r[0]} → ${r[1] || ''} —— tools/${fn},拆带后忘了跟改`);
  }
}
 ...
 * 基线**不会自动写入**:新增命中就是红,要么改内容,要么有意识地手工加进基线文件。 */
{
  ...
  /* 79c. 同一条带内,大事记描述与鼎盛段正文不许逐字重合 >=14 字(v296;2026-08-22)。
     这条边此前无人管,而它是补带时最容易新造重复的地方——补一条带往往同时写大事记描述与鼎盛段,
     两段就在眼前却不会互相比。实证:2026-08-22 一天之内我在这条边上造了四处重复,
     其中朱罗 1010/919 两对是**同一个 commit 里前后脚写的**;更讽刺的是,给那两对做「去重修正」时
     又新造了两处(西汉 -167 二十字、朱罗 919 十九字)——修重复的动作本身在制造重复。
     阈值 14 与规则 78 一致(都是「逐字重合」这类判据)。装上时存量 0。
     ⚠ 降到 10 字会多出约 10 处存量(勃兰登堡波茨坦敕令、蒲甘妙齐提碑、阿瓦·勃固大金塔等,
     逐条读过多为真重复但都是旧文本)——要清那一批得单独开一轮,别顺手改阈值。
     反例验证(2026-08-22):把西汉 -167 的描述改回与「文景之治」段共享那句长的 → 红,报出 20 字重合 ✓。 */
  {
    const nz2 = t => t.replace(/[\s,，。;；:：、「」《》()（）——\-·]/g, '');
    const lcs2 = (a, b) => { let best = ''; for (let i = 0; i < a.length; i++) { let l = best.length;
      while (i + l + 1 <= a.length && b.includes(a.slice(i, i + l + 1))) { l++; best = a.slice(i, i + l); } } return best; };
    CIVS.forEach(c => {
      const gl = glOf(c), ch = CHRONO[c.n] || [];
      ch.forEach(it => {
        const d = nz2(zh1(it[2])); if (d.length < 8) return;
        gl.forEach(g => {
          const w = lcs2(d, nz2(zh1(g.d)));
          if (w.length < 14) return;
          const key = `${c.n}|${it[0]}|${zh1(g.t)}`;
          if (inBL('chronoGlPairs', key)) return;
          P.push(`[大事记与鼎盛段重复] ${c.n} ${it[0]}「${zh1(it[1])}」的描述与鼎盛段「${zh1(g.t)}」重合 ${w.length} 字:「${w}」—— 补一条带时这两处常常前后脚写、却不会互相比;大事记讲「那一年发生了什么」,鼎盛段讲「那段时间什么状态最好」,细节归鼎盛段。确属必要重复就把 "${key}" 加进 tools/.crossband-baseline.json 的 chronoGlPairs`);
        });
      });
    });
  }
...
/* 83. 大事记的描述不许只有一种语言(v311;2026-08-23)。
 *
 * 为什么存在:补孔雀王朝时偶然发现 -321「旃陀罗笈多夺位」的英文描述是空字符串,一查**全站有 61 条**。
 * 这个站是双语的,孩子(或他爸)切到英文视角看到的就是一片空白——而中文视角下一切正常,所以没人会发现。
 * 同一批还查了六问、钩子句、摘要、鼎盛段、人物卡、成就卡五层,**全都是齐的,只有大事记这一层漏**;
 * 推测是当年录大事记时描述字段本就选填,零散补内容时只补了顺手的那一边。
 *
 * 这条和 79-82 不同:**它是纯机械判定,不涉语义,精度就是 100%**,所以直接进红灯通道、不进 warn。
 * 装上时存量已清零(v311 一次补齐 61 条),此后再漏一条就报红。
 * 反例验证(2026-08-23):把任意一条描述的英文改回 '' → 红 ✓;改回来 → 绿 ✓。
 *
 * ⚠ 注意判据是「一边有、另一边空」。**整个描述字段缺失(e[2] 不存在)是允许的**——
 * 有些大事记就是只有标题没有描述,那是有意的,不该被这条规则波及。 */
for (const civ in CHRONO) for (const e of CHRONO[civ]) {
  if (!e[2]) continue;
  const zh = (e[2][0] || '').trim(), en = (e[2][1] || '').trim();
  if (zh && !en) P.push(`[大事记描述缺英文] ${civ} @${e[0]}「${e[1][0]}」中文有「${zh.slice(0, 18)}」,英文为空`);
  if (!zh && en) P.push(`[大事记描述缺中文] ${civ} @${e[0]}「${e[1][0]}」英文有「${en.slice(0, 24)}」,中文为空`);
}
