...
- **v13、v14 已 push**(origin/main = `690ee00`);**v15、v16、v17 已 commit,待 Ray 手动 push**
- 数据规模:112 个文明 · 820 条大事记 · 158 段鼎盛区间(覆盖 104/112)· 156 条古地名今址 · 22 个交流事件 · 10 条传播轨迹
- 功能全景见 docs/CHANGELOG.md(v0-v17);设计理由见 docs/DESIGN.md;改数据前必读 docs/DATA.md
- 仓库实名是大写 C 的 `Civilization-river`(小写链接自动重定向)

## 已完成(本次会话 v13 → v17)

全部改在 `index.html` + `docs/` + 新增 `tools/audit.js`,五个 commit。
...
3. **八条泳道的断流已全部扫完**(v15 扫中亚,v16 扫其余七条)。**中亚已补齐;另有五处空场尚未处理**,按价值排序:
   - 南亚 `-1900~-1475`(450 年):印度河文明衰亡 → 吠陀时代之间。可延长印度河文明的尾巴表现晚期哈拉帕,或新增色带
   - 欧洲 `-1100~-800`(325 年):希腊黑暗时代。可把古希腊的起点前移到 -1100 加一条极薄的尾巴(荷马史诗与希腊字母正出自这段)
   - 南亚 `-175~25`(225 年):孔雀王朝崩溃 → 贵霜之间。**百乘王朝(-230~220)正好整段覆盖**,还可考虑印度-希腊王国(弥兰陀王)
   - 美洲 `-400~-275`(150 年):奥尔梅克 → 玛雅之间。**查文文化(-900~-200)是安第斯的"母文化"**;顺带一提美洲泳道 600 年之前完全没有南美文明,查文/纳斯卡/莫切都缺席
   - 撒南非洲 `1605~1655`(75 年)与 南亚 `550~600`(50 年):都是小口子,调相邻色带的 k 起止即可
...
3. **GL 标签压在色条上的兜底分支触发率 41%**(渲染器 `glHTML`,当标签在色条左右都放不下时落到 `tx=2`,压在色条上,靠 surface 描边晕圈保可读)。**这是 v11/v12 就有的老行为,不是 v13 引入的**,但 v13 把 GL 段数翻倍,等于把它放大了。DESIGN.md 的铁律明确说"文本永远用 ink 色 + surface 描边晕圈",所以没有擅自改渲染器。Ray 若觉得难看,可选:加宽面板(现 330px)/ 缩短标题 / 允许标签换行。
